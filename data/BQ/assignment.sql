...
CREATE TABLE IF NOT EXISTS `indigo-tracker-419714.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `indigo-tracker-419714.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `indigo-tracker-419714.silver_dataset.claims` c
